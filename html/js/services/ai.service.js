const FORMAT_LABELS = { instagram: 'Post para Instagram', blog: 'Artículo de blog', youtube: 'Guion de YouTube', email: 'Email', seo: 'Contenido SEO' };
const TONE_DESC = { profesional: 'profesional y corporativo', divertido: 'divertido y cercano', formal: 'formal y serio', creativo: 'creativo e innovador' };
...
export class AIService {
    ...
    async mockGenerate(prompt, tone, format) {
        ...
        const topic = prompt.replace(/^(crea|escribe|genera|haz|realiza|elabora|redacta|prepara)\s*(un|una|unos|unas|me|te|le|nos)?\s*(guion|guía|artículo|post|contenido|texto|documento|email|correo|publicación|lista|resumen|informe)?\s*/i, '').replace(/^(sobre|acerca de|de|para|acerca)\s*/i, '').trim();
